...
bias_station: 'livorno_porto', bias_quota: 2,
ports: {
  ...
  try {
    ...
    async function swProcessStation(swSt) {
      try {
        ...
        var swTimer = setTimeout(function(){ swCtrl.abort(); }, 6000);
        var swHtml;
        try {
        ...
        // Fetch OM e AROME in parallelo tra loro (non sequenziali) per risparmiare tempo nel budget totale
    ...
    async function sw2ProcessStation(swSt) {
      try {
        ...
        if (swSt.parser === 'windfinder') {
          ...
          var swMsDirMatch = swHtml.match(/Velocit&agrave;\s*attuale:?[\s\S]{0,250}?class="valor2">\s*kt\s*<strong>\s*([NSEW]{1,3})\s*<\/strong>/i);
          var swMsGustMatch = swHtml.match(/Raffica\s*giornaliera:?[\s\S]{0,250}?([\d.]+)\s*<\/strong>\s*kt/i);
          swKn = swMsSpeedMatch ? parseFloat(swMsSpeedMatch[1]) : null;
          swDirTxt = swMsDirMatch ? swMsDirMatch[1].trim().toUpperCase() : null;
          swGustKn = swMsGustMatch ? parseFloat(swMsGustMatch[1]) : null;
          var swMsDirMap = { 'N':0,'NNE':22,'NE':45,'ENE':67,'E':90,'ESE':112,'SE':135,'SSE':157,'S':180,'SSW':202,'SW':225,'WSW':247,'W':270,'WNW':292,'NW':315,'NNW':337 };
...
// campo "ws" letto oggi da scrape_web2 per Livorno Porto non sia il valore
// istantaneo mostrato a video. Confermato 28/8: la pagina contiene una
// grande tabella storica/previsionale (decine di coppie ws/wd, ciascuna
// presumibilmente con un proprio orario), non un singolo dato -- il primo
// "ws"/"wd" trovato nella pagina appartiene per puro caso a un widget di
// conversione unita' di misura di esempio (label "Wind speed tables and
// conversions"), non all'osservazione reale. v2.14.21: invece di prendere
// il primo ws/wd della pagina, cerca TUTTE le occorrenze di "dtl" (etichetta
// orario) e per ciascuna guarda il testo vicino (finestra 400 caratteri) per
// il ws/wd piu' prossimo -- poi sceglie il record con l'orario piu' vicino
// ad ADESSO. Nessuna certezza che la struttura sia questa (non verificabile
...
if (action === 'windfinder_raw_check') {
  try {
    ...
    // Vecchio parsing (identico a scrape_web2) -- tenuto per confronto, ma
    // ormai sappiamo che prende il primo ws/wd della pagina, non "adesso"
    var wrcProdSpeed = wrcHtml.match(/&quot;ws&quot;:\[0,([\d.]+)\]/);
    var wrcProdDir = wrcHtml.match(/&quot;wd&quot;:\[0,([\d.]+)\]/);
    var wrcProdGust = wrcHtml.match(/&quot;wg&quot;:\[0,([\d.]+)\]/);

    // Nuovo tentativo: ogni "dtl" (orario) e il ws/wd piu' vicino a lei nel testo
    var wrcDtlRe = /&quot;dtl&quot;:\[0,&quot;([^&]+)&quot;\]/g;
    var wrcRecords = [];
    var wrcDtlM;
    while ((wrcDtlM = wrcDtlRe.exec(wrcHtml)) !== null) {
      var wrcIdx = wrcDtlM.index;
      var wrcWindow = wrcHtml.slice(Math.max(0, wrcIdx - 400), Math.min(wrcHtml.length, wrcIdx + 400));
      var wrcWsNear = wrcWindow.match(/&quot;ws&quot;:\[0,([\d.]+)\]/);
      var wrcWdNear = wrcWindow.match(/&quot;wd&quot;:\[0,([\d.]+)\]/);
      wrcRecords.push({
        time_text: wrcDtlM[1],
        wind_kt_nearby: wrcWsNear ? parseFloat(wrcWsNear[1]) : null,
        direction_nearby: wrcWdNear ? Math.round(parseFloat(wrcWdNear[1])) : null
      });
    }
    var wrcNowMs = Date.now();
    var wrcBest = null, wrcBestDiffMin = Infinity;
    wrcRecords.forEach(function(r){
      var t = Date.parse(r.time_text);
      if (isNaN(t)) return;
      var diffMin = Math.abs(t - wrcNowMs) / 60000;
      if (diffMin < wrcBestDiffMin) { wrcBestDiffMin = diffMin; wrcBest = r; }
    });
    ...
    return res.status(200).json({
      ...
      old_first_match_in_page: {
        ...
        nota: 'quasi certamente NON il dato attuale -- vedi closest_to_now'
      },
      closest_to_now: wrcBest ? {
        time_text: wrcBest.time_text,
        wind_kt: wrcBest.wind_kt_nearby,
        direction: wrcBest.direction_nearby,
        minutes_from_now: Math.round(wrcBestDiffMin)
      } : null,
      records_found: wrcRecords.length,
      all_records: wrcRecords,
...
// JSON richiesto: {"id":"nome-breve", "text":"..."}. Tetto 5MB per nota
// (limite reale di Upstash: 10MB/richiesta, 100MB/valore -- 5MB lascia
// margine per l'incapsulamento JSON. Il tetto sui comandi Redis, causa
// dell'incidente di agosto, riguardava il NUMERO di comandi, non la
// dimensione di uno singolo -- alzare questo valore non lo rischia).
// Indice separato (note_index, tetto 200 voci, le piu' vecchie escono) per action=note_list.
if (action === 'note_save') {
  try {
    var nsK = req.query.k || '';
    ...
    if (nsText.length > 5000000) return res.status(400).json({ error: 'Testo troppo lungo (tetto 5MB), spezzalo in piu note' });
...
if (action === 'grid_rules_init') {
  try {
    ...
    var defaultRules = {
      ...
      '43.00_10.65': {
        allowed_stations: ['populonia_cfr', 'venturina'],
        min_weight: 0.9,
      ...
      '43.50_9.90': {
        ...
        min_weight: 0.85,
        reason: 'Gorgona a 7.6km — peso insufficiente senza regola, stazione affidabile'
      },
      '43.00_9.90': {
        allowed_stations: ['capraia_cfr'],
        min_weight: 0.85,
        reason: 'Capraia CFR a 7.5km — comportamento instabile senza regola'
      },
      '43.00_9.65': {
        allowed_stations: ['capraia_mnw'],
        min_weight: 0.7,
        reason: 'Capraia MNW a 15.4km — stabilizza direzione zona'
      },
      '42.75_10.65': {
...
if (action === 'bias_matrix') {
  try {
    ...
    function bmSlot(ts) {
      ...
      var h = new Date(ts).getUTCHours() + 2; // UTC+2 approssimativo
      if (h >= 24) h -= 24;
      return h >= 6 && h < 14 ? 'morning' : 'afternoon';
    }

    var bmResults = {};
    for (var bmi = 0; bmi < bmStations.length; bmi++) {
      var bmId = bmStations[bmi];
      try {
        var bmSamples = await kvGet('bias_samples:' + bmId, kvUrl, kvToken);
        if (!Array.isArray(bmSamples) || bmSamples.length === 0) {
          bmResults[bmId] = null;
          continue;
        }

        // Filtra campioni con delta valido
        var bmValid = bmSamples.filter(function(s) {
          return s.delta && s.delta.wind_kt !== null &&
                 s.om && s.om.wind_kt !== null && s.om.direction !== null;
        });

        if (bmValid.length === 0) { bmResults[bmId] = null; continue; }

        // Costruisce matrice stratificata
        var bmMatrix = {};
        bmSpeeds.forEach(function(spd) {
          bmSectors.forEach(function(sec) {
            ['morning','afternoon'].forEach(function(slot) {
              var key = spd.key + '_' + sec.key + '_' + slot;
              var filtered = bmValid.filter(function(s) {
                var v = s.om.wind_kt;
                var d = s.om.direction;
                var sl = bmSlot(s.ts);
                return v >= spd.min && v < spd.max && bmInSector(d, sec) && sl === slot;
              });
              if (filtered.length === 0) { bmMatrix[key] = null; return; }
              var biasOm = filtered.reduce(function(a,s){ return a + s.delta.wind_kt; }, 0) / filtered.length;
              var biasAr = null;
              var filteredAr = filtered.filter(function(s){ return s.delta_arome && s.delta_arome.wind_kt !== null; });
              if (filteredAr.length > 0) {
                biasAr = filteredAr.reduce(function(a,s){ return a + s.delta_arome.wind_kt; }, 0) / filteredAr.length;
              }
              bmMatrix[key] = {
                n: filtered.length,
                bias_om: Math.round(biasOm * 100) / 100,
                bias_arome: biasAr !== null ? Math.round(biasAr * 100) / 100 : null,
                // Usa AROME se ha meno bias assoluto di OM e ha abbastanza campioni
                preferred: (biasAr !== null && filteredAr.length >= 5 && Math.abs(biasAr) < Math.abs(biasOm)) ? 'arome' : 'om',
                bias_preferred: (biasAr !== null && filteredAr.length >= 5 && Math.abs(biasAr) < Math.abs(biasOm)) ? Math.round(biasAr * 100) / 100 : Math.round(biasOm * 100) / 100
              };
            });
          });
        });
    ...
    return res.status(200).json({ stations: bmResults, generated_at: new Date().toISOString() });
  } catch(e) {
    return res.status(500).json({ error: e.message });
  }
}
...
if (action === 'obs_save') {
  try {
    ...
    var oName = (req.query.name || '').slice(0, 60);
