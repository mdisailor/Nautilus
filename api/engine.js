// NAUTILUS ENGINE - Vercel API - engine.js - v2.7.9-debug - by mdisailor engine
...
console.log(’[DEBUG] now=’ + now.toISOString() + ’ searching=’ + currentHour + ’ idx=’ + idx + ’ time[0]=’ + (h.time ? h.time[0] : ‘null’) + ’ from_client=’ + (!!omData._from_client));
if (idx === -1) {
console.log(’[DEBUG] idx not found, using 0. Available times: ’ + (h.time ? h.time.slice(0,3).join(’,’) : ‘none’));
idx = 0;
}
...
return res.status(200).json({ ok: true, engine: ‘nautilus-engine’, v: ‘2.7.9-debug’, zones: activeZones, ts: Date.now() });
}
...
console.log(’[DEBUG] action=zone zone=’ + zoneKey + ’ method=’ + req.method + ’ hasBody=’ + !!(req.body) + ’ hasWeatherData=’ + !!(req.body && req.body.weatherData));
if (req.method === ‘POST’ && req.body && req.body.weatherData) {
clientWeatherData = req.body.weatherData;
console.log(’[DEBUG] using clientWeatherData, hourly keys=’ + Object.keys(clientWeatherData.hourly || {}).join(’,’));
}
...
// Fine codice - NAUTILUS ENGINE v2.7.9-debug
