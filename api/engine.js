// NAUTILUS ENGINE · Vercel API · engine.js · v1.0.0 · by mdisailor engine
...
try {
results[zk] = await calcZone(zk);
} catch (err) {
results[zk] = { error: err.message, zone: zk, updated: new Date().toISOString() };
}
}
...
try {
const result = await calcZone(zoneKey);
...
async function calcZone(zoneKey) {
const zone = ZONES[zoneKey];
