...
msg: “🔴 Calo pressione “ + pressure_trend_3h.toFixed(1) + “ hPa/3h — fronte IMMINENTE entro 3-6h” });
} else if (pressure_trend_3h <= -2.0) {
...
msg: “🔴 Vento “ + wind_speed.toFixed(0) + “kn — Forza 7+. Navigazione sconsigliata” });
...
msg: “🔴 Altezza onda “ + wave_height.toFixed(1) + “m — Mare agitato” });
...
msg: “🔴 Delta T “ + deltaT.toFixed(1) + “°C, umidità “ + humidity.toFixed(0) + “% — nebbia probabile” });
...
function extractCurrentData(omData) {
const h = omData.hourly;
const now = new Date();
const currentHour = now.toISOString().slice(0, 13) + “:00”;
let idx = h.time.findIndex(t => t === currentHour);
if (idx === -1) idx = 0;
const prev = Math.max(0, idx - 3); // 3 ore prima per trend

return {
// Correnti
temp_air:         h.temperature_2m[idx] ?? 15,
humidity:         h.relativehumidity_2m[idx] ?? 70,
pressure:         h.surface_pressure[idx] ?? 1013,
wind_speed:       h.windspeed_10m[idx] ?? 0,
wind_dir:         h.winddirection_10m[idx] ?? 0,
wind_gust:        h.windgusts_10m[idx] ?? 0,
cloud_cover:      h.cloudcover[idx] ?? 0,
precipitation:    h.precipitation[idx] ?? 0,
visibility:       (h.visibility?.[idx] ?? 10000) / 1000, // km
wave_height:      h.wave_height?.[idx] ?? 0,
wave_period:      h.wave_period?.[idx] ?? 0,
wave_dir:         h.wave_direction?.[idx] ?? 0,
swell_height:     h.swell_wave_height?.[idx] ?? 0,
swell_period:     h.swell_wave_period?.[idx] ?? 0,
swell_dir:        h.swell_wave_direction?.[idx] ?? 0,
temp_water:       15, // default — aggiornato da Stormglass se disponibile

```
// Trend
pressure_prev:    h.surface_pressure[prev] ?? 1013,
wind_dir_prev:    h.winddirection_10m[prev] ?? 0,
pressure_trend_1h: (h.surface_pressure[idx] ?? 1013) - (h.surface_pressure[Math.max(0,idx-1)] ?? 1013),
pressure_trend_3h: (h.surface_pressure[idx] ?? 1013) - (h.surface_pressure[prev] ?? 1013),
```
...
// ── /run — esegui ciclo completo per tutte le zone ───────────
if (url.pathname === '/run') {
  const results = {};
  ...
  for (const zoneKey of Object.keys(ZONES)) {
    try {
      ...
      const currentData = extractCurrentData(omData);
      ...
      const window = calcOperationalWindow(diagnosis, currentData);
      const alerts = buildAlerts(diagnosis, currentData, localEffects, ports);
      const briefingText = generateBriefingText(
        zone.name, diagnosis, currentData, localEffects, forecast, window, alerts
      );
      ...
      results[zoneKey] = result;

      // Salva in KV se disponibile
      if (env.NAUTILUS_KV) {
        await env.NAUTILUS_KV.put("zone:" + zoneKey, JSON.stringify(result), {
          expirationTtl: 14400 // 4 ore
        });
      }

    } catch (err) {
      results[zoneKey] = { error: err.message, zone: zoneKey, updated: new Date().toISOString() };
    }
  }

  return new Response(JSON.stringify({ ok: true, results, ts: Date.now() }), {
    headers: { ...CORS, 'Content-Type': 'application/json' }
  });
}

// ── /zone/:key — leggi zona da KV ───────────────────────────
if (url.pathname.startsWith('/zone/')) {
  const zoneKey = url.pathname.replace('/zone/', '');
  if (!ZONES[zoneKey]) {
    return new Response(JSON.stringify({ error: 'Zona non trovata' }), {
      status: 404, headers: { ...CORS, 'Content-Type': 'application/json' }
    });
  }

  if (env.NAUTILUS_KV) {
    const cached = await env.NAUTILUS_KV.get("zone:" + zoneKey);
    if (cached) {
  ...
  // Se non in cache, calcola al volo
  const zone = ZONES[zoneKey];
  const omData = await fetchOpenMeteo(zone.lat, zone.lon);
  const currentData = extractCurrentData(omData);
  const diagnosis = diagnoseSynopticCase(currentData);
  ...
  const result = {
    zone: zoneKey, name: zone.name, updated: new Date().toISOString(),
    raw: currentData, diagnosis, local_effects: localEffects,
    forecast, ports, window, alerts, briefing_text: briefingText,
  };
...
if (url.pathname === '/zones') {
  const list = Object.entries(ZONES).map(([key, z]) => ({
    key, name: z.name,
    ports: Object.keys(z.ports).length
  }));
  return new Response(JSON.stringify({ zones: list }), {
    headers: { ...CORS, 'Content-Type': 'application/json' }
  });
}
...
if (url.pathname === '/ping') {
  return new Response(JSON.stringify({ ok: true, engine: 'nautilus-engine', v: '1.0.0', ts: Date.now() }), {
    headers: { ...CORS, 'Content-Type': 'application/json' }
  });
}
...
return new Response(JSON.stringify({
  ...
  endpoints: ['/ping', '/run', '/zones', '/zone/:key']
}), {
  headers: { ...CORS, 'Content-Type': 'application/json' }
});
```
